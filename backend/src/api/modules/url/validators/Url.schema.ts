...
export const urlEncodeRequestSchema = z.object({
  url: z.string().min(1, { message: "url is required" }).url({ message: "Please enter a valid url starting with http:// or https://" }),
});

export const urlDecodeRequestSchema = z.object({
  shortUrl: z.string().min(1, { message: "shortUrl is required" }).url({ message: "Please enter a valid url starting with http:// or https://" }),
});
